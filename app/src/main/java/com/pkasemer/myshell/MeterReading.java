...
public class MeterReading extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        takaApiService = TakaApiBase.getClient(this).create(TakaApiService.class);

    }

    private void permission() {
        if (ContextCompat.checkSelfPermission(MeterReading.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(MeterReading.this, new String[]{Manifest.permission.CAMERA}
                    , REQUEST_CODE);
        } else {
            initViewPager();
        }
    ...
    private void initViewPager() {
        ...
        image_captured = findViewById(R.id.taken_image);
        take_picture = findViewById(R.id.take_image);
    ...
    private void uploadFile() {
        String case_title_text = meter_reading.getText().toString().trim();
        progressBar.setVisibility(View.VISIBLE);

        if (TextUtils.isEmpty(case_title_text)) {
            meter_reading.setError("Provide meter_reading Title");
            meter_reading.requestFocus();
            progressBar.setVisibility(View.GONE);
            return;
        }

        UserModel user = SharedPrefManager.getInstance(MeterReading.this).getUser();
...
//            RequestBody requestFile = RequestBody.create(MediaType.parse("multipart/form-data"), file);
            ...
            call.enqueue(new Callback<FileResponse>() {
                ...
                public void onResponse(Call<FileResponse> call,
                    ...
                    Toast.makeText(MeterReading.this, fileModel.getMessage(), Toast.LENGTH_SHORT).show();
                    progressBar.setVisibility(View.GONE);
                ...
                public void onFailure(Call<FileResponse> call, Throwable t) {
                    Toast.makeText(MeterReading.this, t.getMessage(), Toast.LENGTH_LONG).show();
                    progressBar.setVisibility(View.GONE);
                }
            });
        } else {
...
//        Bitmap bitmap = BitmapFactory.decodeFile(currentPhotoPath, bmOptions);
        File f = new File(currentPhotoPath);
        Uri contentUri = Uri.fromFile(f);
        image_captured.setImageBitmap(handleSamplingAndRotationBitmap(MeterReading.this, contentUri));
    }
